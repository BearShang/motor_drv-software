...
void nvic_config(void)
{
  ...
  /* hall capture configuration*/
  nvic_irq_enable(HALL_CAPTURE_IRQn, 3, 0);
  /* systick nvic init */
  nvic_irq_enable(SysTick_IRQn, 4, 0);
#if defined PWM_INPUT
  /* pwm in interrupt nvic init */
  nvic_irq_enable(PWM_DUTY_INPUT_IRQn, 5, 0);
#endif
  /* usart1 interrupt nvic init */
  nvic_irq_enable(COMM_UART_IRQn, 6, 0);
}
...
void tmr_pwm_init()
{
  ...
  tmr_output_channel_buffer_enable(PWM_ADVANCE_TIMER, TMR_SELECT_CHANNEL_4, TRUE);
...
void gpio_hall_init()
{
  ...
  /* hall timer channel1 configuration */
  ...
  /* hall timer channel2 configuration */
  ...
  /* hall timer channel3 configuration */
...
/**
  * @brief  initialization of a timer for hall sensor
  * @param  none
  * @retval none
  */
void tmr_hall_init()
{
  ...
  tmr_base_init(HALL_CAPTURE_TIMER, MAX_CAP_COUNT, TIM_CAP_CLK_DIV);
  tmr_cnt_dir_set(HALL_CAPTURE_TIMER, TMR_COUNT_UP);

  /* config STCI(TRC) as capture source of C1IN(IC1) */
  ...
  /* timer xor mode configuration of timer ch1, ch2 and ch3 for hall signals trigger */
  tmr_channel1_input_select(HALL_CAPTURE_TIMER, TMR_CHANEL1_2_3_CONNECTED_C1IRAW_XOR);

  /* set hall timer trigger output PTOS(TRGO) from ch2 compare output */
  tmr_primary_mode_select(HALL_CAPTURE_TIMER, TMR_PRIMARY_SEL_C2ORAW);

  /* select the trigger input of timer from C1INC(TI1F_ED) */
...
void tmr_read_emf_init()
{
  tmr_output_config_type tmr_output_struct;

  /* enable timer and gpio clock */
  crm_periph_clock_enable(READ_EMF_CRM_CLK, TRUE);

  tmr_base_init(READ_EMF_TIMER, (PWM_PERIOD - 1), 0);
  tmr_cnt_dir_set(READ_EMF_TIMER, TMR_COUNT_UP);
  ...
  tmr_output_channel_config(READ_EMF_TIMER, TMR_SELECT_CHANNEL_3, &tmr_output_struct);
  tmr_channel_value_set(READ_EMF_TIMER, TMR_SELECT_CHANNEL_3, 0);
  tmr_output_channel_buffer_enable(READ_EMF_TIMER, TMR_SELECT_CHANNEL_3, FALSE);

  tmr_output_channel_config(READ_EMF_TIMER, TMR_SELECT_CHANNEL_4, &tmr_output_struct);
  tmr_channel_value_set(READ_EMF_TIMER, TMR_SELECT_CHANNEL_4, 0);
  tmr_output_channel_buffer_enable(READ_EMF_TIMER, TMR_SELECT_CHANNEL_4, FALSE);
  /* subordinate mode selection: TMR1 */
  tmr_sub_mode_select(READ_EMF_TIMER, TMR_SUB_RESET_MODE);
  tmr_trigger_input_select(READ_EMF_TIMER, TMR_SUB_INPUT_SEL_IS0);  /* TMR1 trigger */

  /* clear flag */
  tmr_flag_clear(READ_EMF_TIMER, TMR_C3_INT | TMR_C4_INT);

  /* enable Channel 3 interrupt of timer */
  tmr_interrupt_enable(READ_EMF_TIMER, TMR_C3_INT, TRUE);
  tmr_interrupt_enable(READ_EMF_TIMER, TMR_C4_INT, FALSE);

  /* output enable */
  tmr_output_enable(READ_EMF_TIMER, FALSE);

  /* enable timer */
  tmr_counter_enable(READ_EMF_TIMER, TRUE);
}
#endif
...
#if !(defined EMF_CONTINOUS_SAMPLE || defined HALL_SENSORS)
  dma_init_type dma_init_struct;
...
void gpio_output_init(void)
{
  ...
  gpio_init_struct.gpio_mode = GPIO_MODE_MUX;
  gpio_init(TMR_ADC_TRIG_PORT, &gpio_init_struct);       /* T1C4 for ADC*/
...
void get_int_vref_cal_ratio(void)
{
  ...
  adc_preempt_conversion_trigger_set(ADC_NORMAL_CONVERTER, ADC_INSTANT_TRIG_SOURCE, ADC_INSTANT_TRIG_EDGE);
  /* disable part mode */
...
void motor_parameter_ID_config(void)
{
  ...
  tmr_output_struct.oc_mode = TMR_OUTPUT_CONTROL_PWM_MODE_B;
  adc_preempt_channel_length_set(ADC_INSTANT_CONVERTER, 1);
  adc_preempt_channel_set(ADC_INSTANT_CONVERTER, CURR_BUS_ADC_CH, 1, ADC_CURR_SAMPLETIME);
  tmr_output_struct.occ_output_state = FALSE;
