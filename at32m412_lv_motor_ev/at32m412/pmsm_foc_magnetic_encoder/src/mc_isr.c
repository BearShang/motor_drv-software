...
void ADVTMR_PWM_CYCLE_IRQ(void)
{
  ...
  if (tmr_flag_get(PWM_ADVANCE_TIMER, TMR_OVF_FLAG) != RESET)
  {
    ...
    if ((PWM_ADVANCE_TIMER->ctrl1_bit.cnt_dir & TMR_COUNT_DOWN) == RESET)  /* underflow interrupt */
    {
      ...
      {
        if(ctrl_mode == OPEN_LOOP_CTRL && esc_state != ESC_STATE_AUTO_LEARN)
        {
...
#if defined ANGLE_CALIBRATION
        else if(ctrl_mode == OPEN_LOOP_CTRL && esc_state == ESC_STATE_AUTO_LEARN)
        {
          enc_rotor_angle_get(&encoder, &rotor_angle_encoder);
          ...
          else if(encoder.align == SUCCEED)
          {
            record_angle_deviation();
          }
          /* svpwm function */
          svpwm_func(&volt_cmd, &pwm_duty);
        }
        ...
        else
        {
          ...
          {
#if defined ANGLE_CALIBRATION
            rotor_angle_encoder.elec_angle_val = enc_rotor_angle_get(&encoder, &rotor_angle_encoder) + arm_linear_interp_q15((int16_t*)shortEncoderTable, encoder.count*table_interp_coeff, INT16_SIZE_ARRARY);
            if(rotor_angle_encoder.elec_angle_val < 0)
              rotor_angle_encoder.elec_angle_val += 32767;
#else
            rotor_angle_encoder.elec_angle_val = enc_rotor_angle_get(&encoder, &rotor_angle_encoder);
#endif

            elec_angle_val = rotor_angle_encoder.elec_angle_val;
...
/**
  * @brief  timer interrupt handler for break event and incremental encoder capturing event
  * @param  none
  * @retval none
  */
void ADVTMR_PWM_BRK_IRQ(void)
{
  ...
  if (tmr_flag_get(SYNC_TIMER, SYNC_TIMER_CH_FLAG) != RESET)
  {
    tmr_flag_clear(SYNC_TIMER, SYNC_TIMER_CH_FLAG);
    encoder.count = ReadAngle();
  }
#endif  
}
